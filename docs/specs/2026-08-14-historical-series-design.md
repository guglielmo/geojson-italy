...
| Estimated as region-partitioned GeoParquet | 60–90 MB |
...
├── temporal/
│   ├── comuni/reg=NN/*.parquet  source of truth, partitioned by region
│   └── SCHEMA.md
└── scripts/
    ├── build_temporal.py       MAPS -> temporal dataset
    └── materialize.py          temporal dataset + date -> comuni.geojson
```
...
Partitioning by region is what keeps git viable. Parquet is binary and rewrites wholesale, so
a single-file dataset would add its full weight to history on every release. Partitioned, a
release touching only Sardinia rewrites `reg=20` alone. Hive-style partitioning is also what
DuckDB reads natively.

### How snapshots reach consumers

Three levels, in order of directness:

1. **The current vintage** stays at the repository root, at its existing URLs (D6). Most
   consumers need nothing else.
2. **Notable dates** — 1 January of each year in the series — are published as assets on a
   GitHub Release per date, generated by `materialize.py`. Release asset URLs are stable and
   CDN-backed, so pinning them is safe.
3. **Any arbitrary date** is materialised locally by running `materialize.py` against the
   tracked dataset. This is the case that requires no publishing decision from the maintainer
   and no round trip.
...
from what ISTAT published for those reference dates, and both are worth keeping.
